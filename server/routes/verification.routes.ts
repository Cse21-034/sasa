...
export function registerVerificationRoutes(app: Express): void {
   ...
   * Submit identity or document verification — all roles go through admin review
   */
  app.post("/api/verification/submit", authMiddleware, async (req: AuthRequest, res) => {
    try {
      ...
      // Notify admin for every identity or document submission
      await notificationService.createAdminNotification({
        title: 'New Verification Submission',
        message: `A ${submittingUser!.role} has submitted for ${validatedData.type} verification and is awaiting review.`,
        type: 'new_verification',
      });

      res.status(201).json({
        message: `${validatedData.type} submission received. An admin will review your submission shortly.`,
        submission,
      })
    } catch (error: any) {
