...
export default function SupplierDashboard() {
  ...
  const stats = [
    { label: 'Total Promotions', value: totalPromos,   icon: Package,    bg: 'bg-orange-100 dark:bg-orange-900/30',  iconColor: 'text-orange-500' },
    { label: 'Active Promos',    value: activePromos,  icon: BadgeCheck, bg: 'bg-emerald-100 dark:bg-emerald-900/30', iconColor: 'text-emerald-500' },
    { label: 'Rating',           value: rating.toFixed(1), icon: Star,   bg: 'bg-yellow-100 dark:bg-yellow-900/30',  iconColor: 'text-yellow-500' },
    { label: 'Reviews',          value: profile?.reviewCount ?? 0, icon: TrendingUp, bg: 'bg-blue-100 dark:bg-blue-900/30', iconColor: 'text-blue-500' },
  ];
      ...
      <div className="w-full" style={{ background: 'linear-gradient(135deg, #1a3a3a 0%, #274345 60%, #2a4d4f 100%)' }}>
        <div className="max-w-7xl mx-auto px-4 py-8 md:py-10">
          <div className="flex flex-col sm:flex-row items-start sm:items-center gap-5">
            {/* Logo */}
            <div className="w-20 h-20 md:w-24 md:h-24 rounded-2xl bg-white/10 border-2 border-white/20 flex items-center justify-center flex-shrink-0 overflow-hidden">
              {profile?.logo
            ...
            <div className="flex-1 min-w-0">
              <div className="flex flex-wrap items-center gap-2 mb-1">
                ...
                {profile?.featured && (
                  <span className="text-[11px] font-bold bg-orange-500 text-white px-2 py-0.5 rounded-full">Featured</span>
                )}
              </div>
              ...
              <div className="flex items-center gap-1.5 mb-2">
                {[1,2,3,4,5].map((s) => (
                  <Star key={s} className={`h-4 w-4 ${s <= Math.round(rating) ? 'fill-yellow-400 text-yellow-400' : 'text-white/30'}`} />
                ))}
                <span className="text-white/80 text-sm ml-1">{rating.toFixed(1)} · {profile?.reviewCount ?? 0} reviews</span>
              </div>
              ...
              {profile?.industryType && (
                <div className="mt-2">
                  <span className="text-[11px] bg-white/10 text-white/80 px-2 py-0.5 rounded-full border border-white/20">
                    {profile.industryType}
                  </span>
                </div>
              )}
            ...
            <Link href="/supplier/settings">
              <Button variant="outline" size="sm" className="border-white/30 text-white hover:bg-white/10 bg-transparent flex-shrink-0">
      ...
      <div className="max-w-7xl mx-auto px-4 py-6 md:py-8 space-y-8">
        ...
        <div>
          <div className="flex items-center justify-between mb-4">
            <div>
              <h2 className="text-xl font-bold text-foreground">My Promotions</h2>
              <p className="text-sm text-muted-foreground">Create and manage special offers</p>
            </div>
            <Button onClick={() => setShowPromotionDialog(true)} className="bg-primary hover:bg-primary/90 text-white">
          ...
          {isLoading ? (
            ...
            <div className="grid md:grid-cols-2 xl:grid-cols-3 gap-5">
              {promotions.map((promo: any) => (
                <Card key={promo.id} className="border border-border/50 shadow-sm overflow-hidden">
                  ...
                  <CardContent className="p-5">
                    <div className="flex items-start justify-between gap-2 mb-2">
                      <h3 className="font-bold text-foreground leading-snug">{promo.title}</h3>
                      {promo.discountPercentage && (
                        <span className="flex items-center gap-0.5 text-[11px] font-bold bg-orange-100 dark:bg-orange-900/30 text-orange-600 dark:text-orange-400 px-2 py-0.5 rounded-full flex-shrink-0">
                    ...
                    <div className="flex items-center gap-2">
                      <Badge variant={promo.isActive ? 'default' : 'secondary'} className="text-[11px]">
      ...
      <Dialog open={showPromotionDialog} onOpenChange={(o) => !o && resetForm()}>
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div><Label>Promotion Title *</Label><Input value={formData.title} onChange={(e) => setFormData({ ...formData, title: e.target.value })} placeholder="e.g., Summer Sale – 20% Off" required /></div>
            <div><Label>Description *</Label><Textarea value={formData.description} onChange={(e) => setFormData({ ...formData, description: e.target.value })} placeholder="Describe your promotion..." className="min-h-24" required /></div>
            ...
            <div>
              ...
              <div className="border-2 border-dashed rounded-lg p-6 hover:border-primary/50 transition-colors">
                ...
                <label htmlFor="promo-images" className="flex flex-col items-center cursor-pointer">
                  {uploadingImages ? <Loader2 className="h-10 w-10 animate-spin text-primary mb-2" /> : <Upload className="h-10 w-10 text-muted-foreground mb-2" />}
                ...
                <div className="grid grid-cols-3 gap-3 mt-3">
                  {uploadedImages.map((img, i) => (
                    <div key={i} className="relative group">
                      <img src={img} className="w-full h-28 object-cover rounded-lg" />
            ...
            <div><Label>Terms & Conditions (optional)</Label><Textarea value={formData.termsAndConditions} onChange={(e) => setFormData({ ...formData, termsAndConditions: e.target.value })} placeholder="Any terms and conditions..." className="min-h-20" /></div>
            <DialogFooter>
